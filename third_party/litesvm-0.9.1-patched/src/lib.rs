        ...
        let maybe_accounts = account_keys
            ...
            .map(|(i, key)| {
                let account = if solana_sdk_ids::sysvar::instructions::check_id(key) {
                    ...
                    {
                        // Optimization to skip loading accounts only used as top-level programs.
                        // If program cache is stale and the account was removed, avoid panic and
                        // fall back to a default account so execution can fail gracefully.
